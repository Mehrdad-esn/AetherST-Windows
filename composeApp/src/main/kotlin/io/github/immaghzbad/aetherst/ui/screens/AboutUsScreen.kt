...
private const val WindowsRepoUrl = "https://github.com/Mehrdad-esn/AetherST-Windows"
        ...
        LazyColumn(
            ...
            item {
                AboutInfoCard(
                    ...
                    Spacer(modifier = Modifier.height((14 * scaleFactor).dp))
                    FeatureRow(icon = Icons.Default.SwapVert, title = "System-wide Proxy", description = "Efficiently handles TCP, UDP, and DNS traffic across the system.", scaleFactor = scaleFactor)
            ...
            item {
                AboutInfoCard(
                    ...
                    FeatureRow(icon = Icons.Default.Tune, title = "High Compatibility", description = "Ensures stable performance across all modern systems.", scaleFactor = scaleFactor)
                }
            }
            item {
                AboutInfoCard(
                    ...
                    AboutLinkCard(
                        title = "Windows Port Repository",
                        subtitle = "This Windows port, created with AI assistance by Mehrdad.",
                        url = WindowsRepoUrl,
                        urlColor = Color(0xFF64D2FF),
                        onClick = { uriHandler.openUri(WindowsRepoUrl) },
                        scaleFactor = scaleFactor
                    )
                    Spacer(modifier = Modifier.height((8 * scaleFactor).dp))
                    AboutLinkCard(
            ...
            item {
                AboutInfoCard(
                    ...
                    Text(
                        text = "This Windows version was created entirely by the AI assistant DeepSeek v4 Flash (opencode) at the request of Mehrdad (@Mehrdad-esn) - no code was written by a human. The Android source was ported to Compose Multiplatform, re-architected for desktop and verified with automated tests.",
                        style = MaterialTheme.typography.bodyMedium,
                        color = IosSecondaryLabel,
                        lineHeight = (22 * scaleFactor).sp,
                        fontSize = (14 * scaleFactor).sp
                    ...
                    Spacer(modifier = Modifier.height((14 * scaleFactor).dp))
                    FeatureRow(icon = Icons.Default.SwapVert, title = "Ported from Android", description = "Jetpack Compose UI migrated to Compose Multiplatform for Windows.", scaleFactor = scaleFactor)
                    FeatureRow(icon = Icons.Default.Tune, title = "Desktop Integration", description = "System tray, single-instance guard, custom title bar and close-to-tray behavior.", scaleFactor = scaleFactor)
                    FeatureRow(icon = Icons.Default.Router, title = "OpenVPN Hybrid", description = "Extra 5th protocol: routes an OpenVPN client through the app's SOCKS5 proxy tunnel.", scaleFactor = scaleFactor)
                    FeatureRow(icon = Icons.Default.Language, title = "Country Flags", description = "271 bundled SVG flags - emoji flags do not render on Windows.", scaleFactor = scaleFactor)
                    FeatureRow(icon = Icons.Default.Security, title = "Verified", description = "33 automated tests covering config, preferences and UI flows.", scaleFactor = scaleFactor)
                }
            }
            item {
                Card(
