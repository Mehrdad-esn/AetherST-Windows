...
object UninstallCleanup {
    ...
    fun getInstallDir(): File? {
        ...
        // Recognize every shipped desktop layout: legacy (AetherST-Tunnel.exe/lib)
        // and current Compose MSI (AetherST.exe with app/ + runtime/ dirs).
        // A stale check here makes needsCleanup() true on every launch and wipes
        // user data (settings/onboarding) in a loop.
        return if (File(installDir, "AetherST-Tunnel.exe").exists() ||
            File(installDir, "AetherST.exe").exists() ||
            File(installDir, "lib").exists() ||
            File(installDir, "app").exists() ||
            File(installDir, "runtime").exists()
