...
/**
 * Hard maintenance block: message only, retry re-checks the feed.
 * No dismiss, no download button, no path into the app.
 */
@Composable
fun MaintenanceScreen(
    ...
    Column(
        ...
        Column(
            ...
            Spacer(modifier = Modifier.height(if (isDesktop) 48.dp else 72.dp))

            Box(
                modifier = Modifier
                    .size((80 * scaleFactor).dp)
                    .background(
                        Brush.linearGradient(listOf(Color(0xFFFF9F0A), Color(0xFFFF375F))),
                        RoundedCornerShape((20 * scaleFactor).dp)
                    ),
                contentAlignment = Alignment.Center
            ) {
        ...
        Column(
            modifier = Modifier
                ...
                .background(Color.Black)
                .padding(horizontal = (24 * scaleFactor).dp)
                .padding(bottom = (16 * scaleFactor).dp + if (isDesktop) 0.dp else WindowInsets.navigationBars.asPaddingValues().calculateBottomPadding()),
            horizontalAlignment = Alignment.CenterHorizontally
        ) {
            OutlinedButton(
                onClick = onRetry,
                ...
                Text("تلاش مجدد / Retry", fontWeight = FontWeight.Bold, fontSize = (16 * scaleFactor).sp, color = Color.White)
            }
        }
    }
}
