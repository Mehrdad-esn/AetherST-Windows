...
enum class AetherProtocol(val rawValue: String, val displayName: String, val description: String) {
    ...
    // Windows-only extension (original work by this port, core runs in WG proxy mode):
    // OpenVPN client chained over the app SOCKS proxy. Not part of upstream.
    OPENVPN("openvpn", "OpenVPN Hybrid", "OpenVPN over app proxy tunnel (Windows only)")
}
...
data class AetherConfig(
    ...
    // 1.7.3 defaults per user request: Bypass=Balanced, Speed=Turbo, Capsule button.
    val noise: AetherNoise = AetherNoise.BALANCED,
    ...
    // Windows-only OpenVPN Hybrid settings (this port's original addition).
    val openVpnConfigPath: String = "",
