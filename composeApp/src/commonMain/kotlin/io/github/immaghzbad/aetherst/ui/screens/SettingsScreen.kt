...
@Composable
private fun SettingsSubPage(page: SettingsPage, config: AetherConfig, isBatteryOptimized: Boolean, onBack: () -> Unit, onUpdateConfig: (AetherConfig) -> Unit, onUpdateTunnelEngine: (TunnelEngine) -> Unit, onApplyPreset: (String) -> Unit, onOpenSplitTunneling: () -> Unit, onOpenRoutingRules: () -> Unit, onRequestBatteryOptimization: () -> Unit, onOpenVpnSettings: () -> Unit, onResetAll: () -> Unit, onExportBackup: () -> Unit, onImportBackup: () -> Unit,     onOptimizeMtu: () -> Unit, isOptimizingMtu: Boolean, onShowToast: (String, Boolean) -> Unit, bottomContentPadding: Dp, onOpenDnsOptimizer: () -> Unit = {}, onPickOvpnFile: () -> Unit = {}) {
    ...
    Column(modifier = Modifier.fillMaxSize().background(Color.Black).clickable(interactionSource = remember { androidx.compose.foundation.interaction.MutableInteractionSource() }, indication = null) { focusManager.clearFocus() }) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = PaddingValues(start = 16.dp, end = 16.dp, bottom = bottomContentPadding + 12.dp, top = 4.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            when (page) {
                ...
                SettingsPage.PROTOCOL -> item { ProtocolPage(config, onUpdateConfig, onOptimizeMtu, isOptimizingMtu, onPickOvpnFile) }
    ...
    IosGroupCard { Column {
        ...
        if (isDesktop) {
            ...
            Row(modifier = Modifier.fillMaxWidth().background(AppPalette.statusConnected.copy(alpha = 0.08f)).padding(10.dp), verticalAlignment = Alignment.Top) {
                ...
                Text(strings.TOR_DESKTOP_UNAVAILABLE, color = AppPalette.statusConnected, fontSize = 11.sp, lineHeight = 14.sp)
            }
            if (config.psiphonEnabled) {
                AppDivider()
                IosSwitchRow(icon = Icons.Default.Shield, iconBg = IosActiveGreen, title = strings.PSIPHON_ONLY, subtitle = strings.PSIPHON_ONLY_SUB, checked = config.psiphonOnly, onCheckedChange = { onUpdateConfig(config.copy(psiphonOnly = it)) }, testTag = "switch_psiphon_only_desktop")
            }
        }
...
@Composable private fun ProtocolPage(config: AetherConfig, onUpdateConfig: (AetherConfig) -> Unit, onOptimizeMtu: () -> Unit, isOptimizingMtu: Boolean, onPickOvpnFile: () -> Unit = {}) {
    val strings = LocalAppStrings.current
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        IosGroupCard { Column {
            IosPickerRow(icon = Icons.Default.VpnLock, iconBg = IosActiveBlue, title = strings.TRANSPORT_PROTOCOL, value = config.protocol.displayName, options = AetherProtocol.entries.filter { isDesktop || it != AetherProtocol.OPENVPN }.map { it.displayName }, onOptionSelected = { val visible = AetherProtocol.entries.filter { isDesktop || it != AetherProtocol.OPENVPN }; onUpdateConfig(config.copy(protocol = visible[it])) })
            ...
            // 1.7.3: Balanced bypass is available for every protocol (user request).
            val availNoise = if (config.protocol == AetherProtocol.MASQUE) listOf(AetherNoise.BALANCED, AetherNoise.FIREWALL, AetherNoise.GFW, AetherNoise.OFF) else listOf(AetherNoise.BALANCED, AetherNoise.AGGRESSIVE, AetherNoise.LIGHT, AetherNoise.OFF)
        ...
        if (config.protocol == AetherProtocol.OPENVPN && isDesktop) {
            IosGroupCard { Column {
                Row(modifier = Modifier.fillMaxWidth().background(AppPalette.statusConnected.copy(alpha = 0.12f)).padding(10.dp), verticalAlignment = Alignment.Top) {
                    Icon(Icons.Default.Info, null, tint = AppPalette.statusConnected, modifier = Modifier.size(16.dp))
                    Spacer(modifier = Modifier.width(8.dp))
                    Text("Windows only: the Aether core runs in WireGuard proxy mode and the OpenVPN client chains over 127.0.0.1 SOCKS. Choose your .ovpn file below.", color = AppPalette.statusConnected, fontSize = 11.sp, lineHeight = 14.sp)
                }
                AppDivider()
                IosInputFieldRow(icon = Icons.Default.VpnLock, iconBg = IosActiveBlue, label = "OpenVPN Config File (.ovpn path)", value = config.openVpnConfigPath, onValueChange = { onUpdateConfig(config.copy(openVpnConfigPath = it.trim())) }, placeholder = "C:\\vpn\\client.ovpn", testTag = "openvpn_config_input")
                Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 10.dp), horizontalArrangement = Arrangement.End) {
                    Button(onClick = onPickOvpnFile, modifier = Modifier.height(42.dp), shape = RoundedCornerShape(10.dp), colors = ButtonDefaults.buttonColors(containerColor = IosActiveBlue.copy(alpha = 0.15f), contentColor = IosActiveBlue), contentPadding = PaddingValues(horizontal = 16.dp)) { Text("Choose .ovpn file…", fontSize = 13.sp, fontWeight = FontWeight.Bold) }
                }
                AppDivider()
                IosInputFieldRow(icon = Icons.Default.Person, iconBg = IosSecondaryLabel, label = "OpenVPN Username (if required)", value = config.openVpnUsername, onValueChange = { onUpdateConfig(config.copy(openVpnUsername = it)) }, placeholder = "user", testTag = "openvpn_username_input"); AppDivider()
                IosInputFieldRow(icon = Icons.Default.Lock, iconBg = IosSecondaryLabel, label = "OpenVPN Password (if required)", value = config.openVpnPassword, onValueChange = { onUpdateConfig(config.copy(openVpnPassword = it)) }, placeholder = "password", testTag = "openvpn_password_input")
            } }
        }
